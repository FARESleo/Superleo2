...
def compute_confidence(instId, bar="1H"):
    ...
    fund_score = 0.5 + np.tanh(funding * 500) / 2.0 if funding else 0.5
    oi_score = 0.5 + (np.tanh(np.log1p(oi)/20.0))/2.0 if oi else 0.5
    cvd_score = 0.5 + np.tanh(cvd / 1e4)/2.0 if cvd else 0.5
    ob_score = (ob_imb + 1)/2.0 if ob_imb else 0.5
    ...
    label = "📈 Bullish" if confidence_pct>=65 else "📉 Bearish" if confidence_pct<=35 else "⚠️ Neutral / Mixed"

    raw = {"price":price,"funding":funding,"oi":oi,"cvd":cvd,"orderbook_imbalance":ob_imb,"backtest_win":bt_win}
...
if st.sidebar.button("Compute Confidence"):
    ...
    cols = st.columns(5)
    emoji_map = {"backtest":"📊","orderbook":"📚","cvd":"📈","oi":"📏","funding":"💰"}
    idx=0
    for k in ["backtest","orderbook","cvd","oi","funding"]:
        ...
        col.metric(label=f"{emoji_map.get(k,k.upper())} {k.upper()}", value=f"{score:.3f}", delta=f"w={weight}")
